"""
Script to extract, transform, and upload programme data to Supabase (APPEND mode)
"""
import sys
...
from src.extract.programme_extractor import ProgrammeExtractor
...
def main():
    """Main execution function for programme data pipeline (APPEND mode)"""
    try:
        logger.info("="*60)
        logger.info("Starting Programme Data ETL Pipeline (APPEND Mode)")
        logger.info("="*60)
        
        # Define file path (using data/raw directory)
        programme_path = project_root / "data" / "raw" / "programme data - adding month.xlsx"
        
        # Check if file exists
        if not programme_path.exists():
            logger.error(f"Programme data file not found: {programme_path}")
            logger.info("Please place 'programme data - adding month.xlsx' in the data/raw/ directory")
            return {'error': 'File not found', 'uploaded': 0}
        
        # Step 1: Extract
        logger.info("Step 1: Extracting programme data...")
        extractor = ProgrammeExtractor()
        df_raw = extractor.extract_from_file(programme_path)
        ...
        BATCH_SIZE = 25  # Smaller batch size for programme data
        ...
        # Summary
        logger.info("\n" + "="*50)
        logger.info("PROGRAMME DATA UPLOAD SUMMARY")
        logger.info("="*50)
