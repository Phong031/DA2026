"""
Script to extract, transform, and upload categories breakdown data to Supabase (APPEND mode)
"""
import sys
...
from src.extract.categories_breakdown_extractor import CategoriesBreakdownExtractor
...
def main():
    """Main execution function for categories breakdown pipeline (APPEND mode)"""
    try:
        logger.info("="*60)
        logger.info("Starting Categories Breakdown Data ETL Pipeline (APPEND Mode)")
        logger.info("="*60)
        
        # Define file path (using data/raw directory)
        categories_path = project_root / "data" / "raw" / "categories breakdown data - adding month.xlsx"
        
        # Check if file exists
        if not categories_path.exists():
            logger.error(f"Categories breakdown file not found: {categories_path}")
            logger.info("Please place 'categories breakdown data - adding month.xlsx' in the data/raw/ directory")
            return {'error': 'File not found', 'uploaded': 0}
        
        # Step 1: Extract
        logger.info("Step 1: Extracting categories breakdown data...")
        extractor = CategoriesBreakdownExtractor()
        df_raw = extractor.extract_from_file(categories_path)
        ...
        BATCH_SIZE = 50  # Standard batch size for categories breakdown
        ...
        for i in range(0, len(rows), BATCH_SIZE):
            ...
            try:
                ...
                if batch_num == 1:
                    ...
                    logger.error(f"   Columns in batch: {list(batch[0].keys()) if batch else 'Empty batch'}")
                    break
        ...
        # Summary
        logger.info("\n" + "="*50)
        logger.info("CATEGORIES BREAKDOWN DATA UPLOAD SUMMARY")
        logger.info("="*50)
