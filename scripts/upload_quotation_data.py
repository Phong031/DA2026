"""
Script to extract, transform, and upload combined quotation data to Supabase (APPEND mode)
"""
import sys
...
def main():
    """Main execution function for quotation data pipeline (APPEND mode)"""
    try:
        logger.info("="*60)
        logger.info("Starting Combined Quotation Data ETL Pipeline (APPEND Mode)")
        logger.info("="*60)
        
        # Define file path (using data/raw directory)
        quotation_path = project_root / "data" / "raw" / "Combined_Quotation - adding month.csv"
        
        # Check if file exists
        if not quotation_path.exists():
            logger.error(f"Quotation file not found: {quotation_path}")
            logger.info("Please place 'Combined_Quotation - adding month.csv' in the data/raw/ directory")
            return {'error': 'File not found', 'uploaded': 0}
        
        # Step 1: Extract
        logger.info("Step 1: Extracting quotation data...")
        extractor = QuotationExtractor()
        df_raw = extractor.extract_from_file(quotation_path)
        ...
        BATCH_SIZE = 100  # Standard batch size for quotation data
        ...
        # Summary
        logger.info("\n" + "="*50)
        logger.info("COMBINED QUOTATION DATA UPLOAD SUMMARY")
        logger.info("="*50)
