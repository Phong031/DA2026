"""
Script to extract, transform, and upload month end financial data to Supabase (APPEND mode)
"""
import sys
...
def main():
    """Main execution function for month end data pipeline (APPEND mode)"""
    try:
        logger.info("="*60)
        logger.info("Starting Month End Data ETL Pipeline (APPEND Mode)")
        logger.info("="*60)
        
        # Define file path (using data/raw directory)
        month_end_path = project_root / "data" / "raw" / "financial report table - adding month.xlsx"
        
        # Check if file exists
        if not month_end_path.exists():
            logger.error(f"Month end data file not found: {month_end_path}")
            logger.info("Please place 'financial report table - adding month.xlsx' in the data/raw/ directory")
            return {'error': 'File not found', 'uploaded': 0}
        
        # Step 1: Extract
        logger.info("Step 1: Extracting month end data...")
        extractor = MonthEndExtractor()
        df_raw = extractor.extract_from_file(month_end_path)
        ...
        # Summary
        logger.info("\n" + "="*50)
        logger.info("MONTH END DATA UPLOAD SUMMARY")
        logger.info("="*50)
