"""
Script to extract, transform, and upload workshop cost data to Supabase (APPEND mode)
"""
import sys
...
def main():
    """Main execution function for workshop cost pipeline (APPEND mode)"""
    try:
        logger.info("="*60)
        logger.info("Starting Workshop Cost ETL Pipeline (APPEND Mode)")
        logger.info("="*60)
        
        # Define file path (using data/raw directory)
        workshop_path = project_root / "data" / "raw" / "Workshop_Cost - adding month.csv"
        
        # Check if file exists
        if not workshop_path.exists():
            logger.error(f"Workshop cost file not found: {workshop_path}")
            logger.info("Please place 'Workshop_Cost - adding month.csv' in the data/raw/ directory")
            return {'error': 'File not found', 'uploaded': 0}
        
        # Step 1: Extract
        logger.info("Step 1: Extracting workshop cost data...")
        extractor = WorkshopCostExtractor()
        df_raw = extractor.extract_from_file(workshop_path)
        ...
        # Summary
        logger.info("\n" + "="*50)
        logger.info("WORKSHOP COST DATA UPLOAD SUMMARY")
        logger.info("="*50)
