"""
Script to extract, transform, and upload company cost data to Supabase (APPEND mode)
"""
import sys
...
from src.extract.company_cost_extractor import CompanyCostExtractor
...
def get_supabase_columns(supabase_client, table_name: str) -> List[str]:
    ...
    try:
        ...
        if response.data:
            ...
            logger.info(f"Found Supabase columns: {columns[:10]}...")
            return columns
        else:
...
def main():
    """Main execution function for company cost pipeline (APPEND mode)"""
    try:
        logger.info("="*60)
        logger.info("Starting Company Cost ETL Pipeline (APPEND Mode)")
        logger.info("="*60)
        
        # Define file path (using data/raw directory)
        company_cost_path = project_root / "data" / "raw" / "Company_Cost - adding month.csv"
        
        # Check if file exists
        if not company_cost_path.exists():
            logger.error(f"Company cost file not found: {company_cost_path}")
            logger.info("Please place 'Company_Cost - adding month.csv' in the data/raw/ directory")
            return {'error': 'File not found', 'uploaded': 0}
        
        # Step 1: Extract
        logger.info("Step 1: Extracting company cost data...")
        extractor = CompanyCostExtractor()
        df_raw = extractor.extract_from_file(company_cost_path)
        ...
        # Summary
        logger.info("\n" + "="*50)
        logger.info("COMPANY COST DATA UPLOAD SUMMARY")
        logger.info("="*50)
