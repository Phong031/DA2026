"""
Programme Data extractor for Excel files
"""
import pandas as pd
...
class ProgrammeExtractor:
    """Extract and validate programme data from Excel files"""
    
    def __init__(self):
        """Initialize programme extractor"""
        pass
    
    def extract_from_file(self, file_path: Path) -> pd.DataFrame:
        """
        Extract programme data from Excel file
        
        Args:
            file_path: Path to Excel file
            
        Returns:
            DataFrame with extracted programme data
        """
        try:
            # Load the Excel file
            logger.info(f"Loading programme data from: {file_path}")
            ...
            logger.error(f"Error extracting programme data from {file_path}: {e}")
            raise
    
    def prepare_for_upload(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Prepare programme data for Supabase upload (APPEND mode)
