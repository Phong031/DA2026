"""
Categories Breakdown Data extractor for Excel files
"""
import pandas as pd
...
class CategoriesBreakdownExtractor:
    """Extract and validate categories breakdown data from Excel files"""
    
    def __init__(self):
        """Initialize categories breakdown extractor"""
        pass
    
    def extract_from_file(self, file_path: Path) -> pd.DataFrame:
        """
        Extract categories breakdown data from Excel file
        
        Args:
            file_path: Path to Excel file
            
        Returns:
            DataFrame with extracted categories breakdown data
        """
        try:
            # Load the Excel file
            logger.info(f"Loading categories breakdown data from: {file_path}")
            ...
            logger.error(f"Error extracting categories breakdown data from {file_path}: {e}")
            raise
    
    def prepare_for_upload(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Prepare categories breakdown data for Supabase upload (APPEND mode)
        ...
        Args:
            df: Raw categories breakdown DataFrame
